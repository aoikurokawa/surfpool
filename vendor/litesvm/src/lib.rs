...
impl LiteSVM {
    ...
    fn set_sysvars(&mut self) {
        ...
        self.set_sysvar(&SlotHistory::default());
        {
            ...
            self.accounts.add_account(StakeHistory::id(), account).unwrap();
        }
        #[allow(deprecated)]
        {
            use solana_stake_interface::config::Config;
            let mut data = bincode::serialize(&0u64).unwrap();
            data.extend(bincode::serialize(&Config::default()).unwrap());
            let mut account = AccountSharedData::new(1, data.len(), &solana_sdk_ids::config::id());
            account.data_as_mut_slice().copy_from_slice(&data);
            self.accounts.add_account_no_checks(solana_sdk_ids::stake::config::id(), account);
        }
    }
